...
//                .reportType(ReportType.VEHICLE_MILEAGE)
    ...
    private List<ReportEntry> calculateMileageByDay(List<GpsPoint> gpsPoints, LocalDate startDate, LocalDate endDate) {
        ...
        for (int i = 1; i < gpsPoints.size(); i++) {
            ...
            double distance = Haversine.calculateDistance(
                    new GpsPointCoord(point1.getLocation().getX(), point1.getLocation().getY()),
                    new GpsPointCoord(point2.getLocation().getX(), point2.getLocation().getY())
            );
    ...
    // Метод для исправления падежа месяца на именительный
    private String fixMonthInNominative(String month) {
        // Массив с правильными именами месяцев в именительном падеже
        ...
        // Проверяем и заменяем месяц, если нужно
        String monthName = month.split(" ")[0]; // Извлекаем только месяц из строки
        return monthCorrections.getOrDefault(monthName, month); // Заменяем на именительный падеж
    }
